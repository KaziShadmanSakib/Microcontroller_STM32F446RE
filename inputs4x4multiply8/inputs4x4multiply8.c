...
int main(){
	...
	while(1){
		...
		// multiplication of the input1 and input2
		int ans = (int) input1ToDecimal * (int) input2ToDecimal;
		
		int binaryMultiply[8];
		
		// turning ans to binary number finally
		for(int i = 0; ans > 0; i++){
			binaryMultiply[i] = ans % 2;
      ans = ans / 2;
    }
		
		//reverse the ans
		int binaryMultiplyFinal[8];
		for(int i=0,j=7;i<8;i++){
			binaryMultiplyFinal[j] = binaryMultiply[i];
			j--;
		}
		
		//answer
		outputCheck(binaryMultiplyFinal);
